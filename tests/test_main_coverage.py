...
def mock_personalities():
    return {
        "sacha": Personality(name="Sacha", byline="", identity=[], behavior=[])
    }
...
def test_get_agent_for_user_fallback_other():
    ...
    other_pers = {"other": Personality(name="Other", byline="", identity=[], behavior=[])}
...
async def test_cli_loop_chat(mock_agent):
    ...
    with patch("builtins.input", side_effect=["sacha", "hi", "quit"]):
        with patch("src.main.Config.load_personalities", return_value={"sacha": MagicMock()}):
            with patch("src.main.get_agent_for_user", return_value=mock_agent):
                with patch("builtins.print") as mock_print:
                    ...
                    # verify output
                    assert any("sacha: Hello user" in str(c) for c in mock_print.call_args_list)
...
async def test_cli_loop_no_personalities():
    with patch("src.main.Config.load_personalities", return_value={}):
        with patch("builtins.print") as mock_print:
            await cli_loop()
            mock_print.assert_called_with("No personalities found in src/personalities/")
    ...
    with patch("builtins.input", side_effect=["sacha", "hi", "quit"]):
        with patch("src.main.Config.load_personalities", return_value={"sacha": MagicMock()}):
            with patch("src.main.get_agent_for_user", return_value=mock_agent):
                with patch("builtins.print") as mock_print:
                    await cli_loop()
                    assert any("Error: Boom" in str(c) for c in mock_print.call_args_list)
...
async def test_handle_message_error():
    ...
    with patch("src.main.get_agent_for_user", side_effect=Exception("Error")):
        ...
        update.message.reply_text.assert_called_with("I'm having trouble thinking right now.")
        ...
        with patch("src.main.Config.load_personalities", return_value={"sacha": "p"}):
             with patch("src.main.create_agent") as mock_create:
                 ...
                 with patch("builtins.print") as mock_print:
                     ...
                     assert any("Chatting with sacha" in str(c) for c in mock_print.call_args_list)
